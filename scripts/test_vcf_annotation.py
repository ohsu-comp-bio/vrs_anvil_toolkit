...
@pytest.mark.skip(
    reason="Issue with test fixtures see https://github.com/ohsu-comp-bio/vrs-python-testing/issues/16"
)
def test_small_vcf_annotation(seqrepo_dir, delete_output, request):
    ...
    for i, expected_record in enumerate(expected_vcf_reader):
        ...
        assert (
            VRS_ERROR_FIELD not in test_record.INFO
        ...
        if isinstance(expected_ids, list):
            assert isinstance(test_ids, list) and len(expected_ids) == len(
                test_ids
            ), f"not the same number of IDs for {variant_str}"

            for expected_id, test_id in zip(expected_ids, test_ids):
                assert expected_id == test_id, f"ids for {variant_str} do not match"
